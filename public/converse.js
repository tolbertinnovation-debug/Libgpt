...
export class VoiceConversation {
  ...
  finishTurn() {
    // An ear that records has no words to send until it has stopped — so for
    // that one, "send now" means stop recording now. The turn follows by
    // itself from the words it brings back.
    if (this.ear?.endpoints && this.state === 'listening') {
      this.ear.stop();
      return undefined;
    }
